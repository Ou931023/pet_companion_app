...
class ConversationDetailScreen extends StatelessWidget {
            ...
            child: Padding(
              ...
              child: Builder(
                builder: (_) {
                  ...
                  final text = mood == null ? '長按這行可刪整段' : '那天$mood　·　長按這行可刪整段';
                  return Text(
                    text,
                    style: TextStyle(
  ...
  Future<void> _confirmDeleteMessage(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            deleteUser ? '只會刪掉你說的這一句，寵物的回覆會留著。' : '只會刪掉寵物說的這一句，你說的話會留著。',
    ...
    final ok = await controller.deleteConversationMessage(turn,
        deleteUser: deleteUser);
    if (!context.mounted || ok) return;
    ElderFeedback.showImportant(context, '沒辦法刪除這一句，請再試一次。');
  }
  ...
  Future<void> _confirmDeleteTurn(
    ...
    if (!context.mounted || ok) return;
    ElderFeedback.showImportant(context, '沒辦法刪除這段，請再試一次。');
  }
}
