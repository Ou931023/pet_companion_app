...
class CompanionReplyStrategyService {
  ...
  CompanionReplyPlan buildPlan(CompanionContext context) {
    ...
    final variationSeed = context.conversationHistory.isEmpty
        ? context.userText
        : '${context.userText}|${context.conversationHistory.length}';
    final previousReply = context.conversationHistory.isEmpty
        ? ''
        : context.conversationHistory.first.petReply;
  ...
  String compose(CompanionReplyPlan plan) {
    ...
    final reply = parts
        ...
        .map((part) => part.trim())
        .join(' ');
    return _limitToThreeSentences(_limitToOneQuestion(reply));
  }
  ...
  String _variation(
    ...
    var index = value.abs() % options.length;
    if (options.length > 1 && previousReply.startsWith(options[index])) {
