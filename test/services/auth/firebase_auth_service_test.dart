  ...
  group('FirebaseAuthService（CR-0006 Batch 4a 雛形）', () {
    ...
    test('Firebase 不可用時 Email 方法丟 EmailAuthException(unavailable)，不碰原生',
      ...
      await expectLater(
        ...
        throwsA(
          isA<EmailAuthException>().having((e) => e.code, 'code', 'unavailable'),
        ),
      );
      await expectLater(
    ...
    test('Firebase 不可用時 signInWithGoogle 丟 GoogleAuthException(unavailable)，不碰原生',
        () async {
      final service = FirebaseAuthService(
        initializer: FirebaseInitializer(initialize: () async {}),
      );
      await expectLater(
        ...
        throwsA(
          isA<GoogleAuthException>().having((e) => e.code, 'code', 'unavailable'),
        ),
      );
    });
